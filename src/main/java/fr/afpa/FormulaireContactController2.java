...
public class FormulaireContactController2 {
    ...
    // il faut éviter au maximu de coder des fonctionnalités dans la méthode
    // initialize ;
    // les foctions spécifiques seront appelées dans des méthodes en de hors de
    // initialize.
    // Sinon : Risque de se perdre au milieu de fonctionnalités essentielles ;
    // casser l'application,
    // ne pas prendre les bons objets en paramètres
    ...
    public void updateList() {
        ...
        if (filter == null || filter.length() == 0) {
            ...
            filteredContacts.setPredicate(p -> p.getContact().getLastName().toLowerCase().contains(filter.toLowerCase()));
        }
